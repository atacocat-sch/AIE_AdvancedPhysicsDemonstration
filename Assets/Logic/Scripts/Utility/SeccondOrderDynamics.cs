...
public sealed class SeccondOrderDynamicsF
{
    ...
    SeccondOrderDynamicsState state;
...
{
    public SeccondOrderDynamicsSettings settings;
    SeccondOrderDynamicsState x;
    SeccondOrderDynamicsState y;
...
public static class SeccondOrderDynamics
{
    ...
    {
        float altTarget = targetPosition < 180.0f ? targetPosition + 360.0f : targetPosition - 360.0f;
    ...
    public static float Process(float position, float velocity, float targetPosition, float? targetVelocity, float dt, SeccondOrderDynamicsSettings settings, SeccondOrderDynamicsState state)
    {
        ...
        float w = 2.0f * Mathf.PI * settings.F;
        float z = settings.Z;
        float d = w * Mathf.Sqrt(Mathf.Abs(z * z - 1));
        ...
        else
        {
            ...
            float alpha = 2.0f * t1 * (z <= 1.0f ? Mathf.Cos(dt * d) : MathF.Cosh(dt * d));
            float beta = t1 * t1;
            float t2 = dt / (1.0f + beta - alpha);
